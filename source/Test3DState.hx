...
class Test3DState extends FlxState {
    ...
    private function updateBasicMovement():Void {
        if (FlxG.keys.pressed.LEFT) sprite.x3D -= 5;
        if (FlxG.keys.pressed.RIGHT) sprite.x3D += 5;
        if (FlxG.keys.pressed.UP) sprite.y3D -= 5;
        if (FlxG.keys.pressed.DOWN) sprite.y3D += 5;
        if (FlxG.keys.pressed.Q) sprite.z3D -= 5;
        if (FlxG.keys.pressed.E) sprite.z3D += 5;
    }

    private function updateRotation():Void {
        if (FlxG.keys.pressed.W) sprite.rotationX -= 2;
        if (FlxG.keys.pressed.S) sprite.rotationX += 2;
        if (FlxG.keys.pressed.A) sprite.rotationY -= 2;
        if (FlxG.keys.pressed.D) sprite.rotationY += 2;
    }
    ...
    private function updateDebugText():Void {
        debugText.text = 
            'Position3D: (${Math.floor(sprite.x3D)}, ${Math.floor(sprite.y3D)}, ${Math.floor(sprite.z3D)})\n' +
