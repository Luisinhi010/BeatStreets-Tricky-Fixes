...
class CharacterTestState extends MusicBeatState
{
    ...
    override public function create()
    {
        ...
        if (characters != null && characters.length > 0) {
            ...
            if (char != null) {
                char.setPosition(
                    mainPanel.x + mainPanel.width/2 - char.width/2,
                    mainPanel.y + mainPanel.height/2 - char.height/2
                );
    ...
    function getAnimCategory(animName:String):String {
        return if (animName.startsWith("sing")) "Singing"
            else if (animName.contains("idle")) "Idle"
            else if (animName.contains("miss")) "Miss"
            else if (animName.contains("hey") || animName.contains("dance")) "Special"
            else "Other";
    }
    ...
    function showStatus(text:String, ?color:FlxColor) {
        if (color == null) color = FlxColor.LIME;
    ...
    function previewAnimation(animName:String) {
        if (!previewActive || animName == currentPreviewAnim) return;
        
        // Limpar preview anterior
        cleanupPreview();
        
        // Criar background do preview
        previewBG = new FlxSprite(sidePanel.x + sidePanel.width + 10, FlxG.height - 180)
            .makeGraphic(200, 160, 0xFF232323);
        previewBG.alpha = 0.8;
    ...
    function toggleSpriteView() {
        ...
        if (spriteViewMode) {
            if (spriteSheet != null) spriteSheet.destroy();
            ...
            var scale = Math.min(
                (FlxG.width * 0.8) / spriteSheet.width,
                (FlxG.height * 0.8) / spriteSheet.height
            );
    ...
    override function update(elapsed:Float)
    {
        ...
        if (!cameraLocked) {
            ...
            if (FlxG.keys.pressed.W) FlxG.camera.scroll.y -= speed;
            if (FlxG.keys.pressed.S) FlxG.camera.scroll.y += speed;
            if (FlxG.keys.pressed.A) FlxG.camera.scroll.x -= speed;
            if (FlxG.keys.pressed.D) FlxG.camera.scroll.x += speed;
        }
        ...
        if (FlxG.keys.pressed.SHIFT) {
            var charSpeed = 2;
            if (FlxG.keys.pressed.UP) char.y -= charSpeed;
            if (FlxG.keys.pressed.DOWN) char.y += charSpeed;
            if (FlxG.keys.pressed.LEFT) char.x -= charSpeed;
            if (FlxG.keys.pressed.RIGHT) char.x += charSpeed;
            updateCharacterInfo();
        }
