...
class Character extends FlxSprite
{
	...
	private function loadCharacterGraphics()
	{
		...
		if (charData.caching)
		{
			...
			if (charData.additionalSprites != null)
		...
		if (isPlayer)
		{
			...
			if (!curCharacter.startsWith('bf') && !curCharacter.toLowerCase().contains('death'))
			{
				...
				if (animation.getByName('singRIGHTmiss') != null)
					swapAnimationFrames('singLEFTmiss', 'singRIGHTmiss');
			}
		}
	...
	override function update(elapsed:Float):Void
	{
		...
		if (isPlayer)
			updatePlayerAnimation(elapsed, holdDuration);
		else
			updateNPCAnimation(elapsed, holdDuration, curAnim);
	}
